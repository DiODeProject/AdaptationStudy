...
DATA_HOME=${HOME}/data/Fig_S1_resample # Absolute path to the the folder where data will be saved
mkdir -p ${DATA_HOME}
...
MAX_QUALITY=1 
UPDATE_RULE="[random]" # opinion update model (here is the voter model)
ZEALOT_OPINION=0
ZEALOT_QUALITY=0.5
...
#USUALLY CHANGE
NUM_AGENT=50 # Number of agents
AGENT_RADIUS=0.1 # Communication range 
NUM_OP=3 # Number of options/sites

UPDATE_MODEL=[direct]
PROBABILISTIC_DISCOVERY_PROPORTION=[resample] # resample exploration rule
...
STANDARD_DEVIATION_VALUE=0 # Qualities estimation noise

# Qualities of the options/sites
QUALITY_VALUES_LIST=([0.8,0.3,0.1] [0.8,0.4,0.1] [0.8,0.5,0.1] [0.8,0.6,0.1] [0.8,0.7,0.1] [0.8,0.8,0.1] [0.8,0.625,0.1] [0.8,0.65,0.1] [0.8,0.675,0.1] [0.8,0.725,0.1] [0.8,0.75,0.1] [0.8,0.775,0.1])

# Values of the resample exploration rule parameter "alpha"
PRE_STEP_SELF_STRENGTH_LIST=([0.4] [0.2] [0.1] [0.01] [0.001] [0.0001])

for PRE_STEP_SELF_STRENGTH in ${PRE_STEP_SELF_STRENGTH_LIST[*]}
do

	CONF_DIR="${DATA_HOME}/conf_cluster_JobArray/${NUM_AGENT}_${PRE_STEP_SELF_STRENGTH}/${EXPERIMENT_TYPE}"
	mkdir -p ${CONF_DIR}


	EXP_RESULT_FOLDER_PATH=${RESULT_FOLDER_PATH}/${NUM_AGENT}_${PRE_STEP_SELF_STRENGTH}/${EXPERIMENT_TYPE}/
	FINAL_RESULT_FILENAME="null"
	mkdir -p ${EXP_RESULT_FOLDER_PATH}

	mkdir -p "${DATA_HOME}/configuration/${NUM_AGENT}_${PRE_STEP_SELF_STRENGTH}/${EXPERIMENT_TYPE}/"

	SPECIAL_TEMPLATE_SETTINGS1="${DATA_HOME}/configuration/${NUM_AGENT}_${PRE_STEP_SELF_STRENGTH}/${EXPERIMENT_TYPE}/configuration.specialtemplate2.config"
	...
	do
		...
		SPECIAL_TEMPLATE_SETTINGS2="${DATA_HOME}/configuration/${NUM_AGENT}_${PRE_STEP_SELF_STRENGTH}/${EXPERIMENT_TYPE}/${QUALITY_VALUES}.specialtemplate1.config"
	    ...
	    sed -e "s|start|${1}|"                          \
		    ...
		    -e "s|JOBNAME|Fig_S1_resample_${QUALITY_VALUES}_${PRE_STEP_SELF_STRENGTH}|"   \
		...
		do
			...
			STATISTICS_FILENAME=statistics_${UPDATE_MODEL}_${PRE_STEP_SELF_STRENGTH}_${QUALITY_VALUES}_${STANDARD_DEVIATION_VALUE}_$((${SIM_NO}-1)).txt
