...
DATA_HOME=${HOME}/data/Fig_S5_resample # Absolute path to the the folder where data will be saved
mkdir -p ${DATA_HOME}
...
NUM_SIMULATION=1
NUM_START_P=1
NUM_STEP=400000 # Number of timesteps
NUM_TOTEM=[1]
OVERLAPPING_TOTEMS="true"
...
#USUALLY CHANGE
NUM_OP=9 # Number of options/sites

UPDATE_MODEL=[direct]
PROBABILISTIC_DISCOVERY_PROPORTION=[resample]
QUALITY_VALUES_AFTER_CHANGE="[0,0,0,0,0,0,0,0,0]"
TOTEMS_APPEARANCE_TIMES=[350000,300000,250000,200000,150000,100000,50000,0,0] # Appearance time of the options/sites
TOTEMS_DISAPPEARANCE_TIMES=[0] # Disappearance time of the options/sites
QUALITY_CHANGE_TIME=0
STANDARD_DEVIATION_VALUE=0.1 # Qualities estimation noise
PRE_STEP_SELF_STRENGTH=[0.01] # Values of the resample exploration rule parameter "alpha"

#Values that iterate for different configuration files
QUALITY_VALUES_LIST=([0.9,0.8,0.7,0.6,0.5,0.4,0.3,0.2,0.1])

# Number of agents values
NUM_AGENT_LIST=(50)

# Agent't communication range values
...
do
	...
	do
        ...
        sed -e "s|start|${1}|"                          \
		    ...
		    -e "s|JOBNAME|Fig_S5_resample_${NUM_AGENT}_${AGENT_RADIUS}|"   \
